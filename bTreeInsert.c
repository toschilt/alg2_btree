...
void insertPageData(bTreePage *bPage, bTreePage *createdPage, long *childsArray, long startingPosition) {
    ...
    //Atualiza os filhos da página, caso necessário
    if(childsArray != NULL) {
...
void bTreeInsertIntoPage(record *newRecord, promotedKey **promoted, bPageInfo *bInfo, long insertPosition) {
    ...
    if(bInfo->bPage->numRecords == MAXCHILDS - 1) {
        ...
        long promotedIndex;
        //Posicionamento da chave inserida e chave promovida
        ...
        *promoted = promoteKey(rec, bInfo->RRN, createdPage->RRN);
        ...
        //Insere o novo registro no nó da esquerda
        ...
        if(*promoted != NULL) {
            //Precisa atualizar os filhos, houve overflow na iteração anterior
